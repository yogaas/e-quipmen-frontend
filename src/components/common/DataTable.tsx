    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {col.sortable && (
                      <div className="flex flex-col">
                        <ChevronUp size={12} className={sortField === col.key && sortOrder === 'asc' ? 'text-primary-500' : 'text-slate-300'} />
                        <ChevronDown size={12} className={sortField === col.key && sortOrder === 'desc' ? 'text-primary-500' : 'text-slate-300'} />
                      </div>
                    )}
                  </div>
                </th>
            ...
            {isLoading ? (
              Array(3).fill(0).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  {columns.map((_, j) => (
                    <td key={j} className="px-6 py-4"><div className="h-4 bg-slate-100 dark:bg-slate-800 rounded w-full"></div></td>
                  ))}
                </tr>
              ))
            ) : data.length === 0 ? (
              ...
              data.map((item, index) => (
                ...
                >
                  {columns.map((col) => (
                    <td key={String(col.key)} className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
      ...
      <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <p className="text-sm text-slate-500">
          Showing <span className="font-medium text-slate-900 dark:text-white">{(currentPage - 1) * pageSize + 1}</span> to <span className="font-medium text-slate-900 dark:text-white">{Math.min(currentPage * pageSize, totalCount)}</span> of <span className="font-medium text-slate-900 dark:text-white">{totalCount}</span> results
        </p>
        ...
        <div className="flex gap-1">
          ...
          <Button variant="outline" size="sm" onClick={() => onPageChange(pageIndex + 1)} disabled={currentPage === totalPages}>
